...
The deployment needs to act on behalf of one Spotify account and one Twitch account during a stream, but nothing in the repo can obtain, store, or keep fresh the tokens those Providers issue. The only Broadcaster-facing route today returns a placeholder behind Cloudflare Access. The Broadcaster has no way to connect an account, see whether a Connection is healthy, or recover when a Provider revokes a refresh token.

## Solution

An Broadcaster Page at `/setup`, reachable only through the existing Cloudflare Access application, shows the state of each Provider's Connection and offers a button to connect or reconnect it. Pressing the button starts a Provider authorization in the browser. After consent the Provider redirects back, the Worker exchanges the code, records which Connected Account was authorized, and sends the Broadcaster back to the Broadcaster Page with a result message. From then on a Durable Object keeps the Connection's access token fresh with a proactive alarm and an on-demand refresh, retries transient failures, and marks the Connection Reauthorization Required when the Provider rejects the refresh token. Future features ask the Connection for a valid access token and never touch OAuth directly.

## User stories

1. As an Broadcaster, I want the Broadcaster Page to require my Cloudflare Access login, so that nobody else can start or complete a Provider authorization.
2. As an Broadcaster, I want any request to an broadcaster route without an Access identity to be refused, so that a misconfigured Access application fails closed rather than open.
3. As an Broadcaster, I want to see one section per Provider on the Broadcaster Page, so that I can tell Spotify and Twitch apart at a glance.
4. As an Broadcaster, I want each Provider section to show whether the Connection is Not Configured, Authorized, or Reauthorization Required, so that I know whether action is needed.
5. As an Broadcaster, I want each Authorized Connection to show its Connected Account, so that I can confirm I authorized the right account.
6. As an Broadcaster, I want each Connection to show the scopes the Provider granted, so that I can notice when a scope was withheld.
7. As an Broadcaster, I want each Connection to show when the access token expires and when the next refresh is scheduled, so that I can judge whether refresh is working.
8. As an Broadcaster, I want each Connection to show the last refresh error, if any, so that I can diagnose a problem without reading logs.
9. As an Broadcaster, I want a Connect button for a Provider that is Not Configured, so that I can authorize it for the first time.
10. As an Broadcaster, I want a Reconnect button for a Provider that is Authorized or Reauthorization Required, so that I can replace the Connection with a fresh authorization.
11. As an Broadcaster, I want the Connect and Reconnect buttons to submit a form rather than follow a link, so that a page I merely visit cannot start an authorization on my behalf.
12. As an Broadcaster, I want pressing Connect to redirect my browser to the Provider's consent screen with the correct scopes, so that I can approve access in one step.
13. As an Broadcaster, I want the callback URL sent to the Provider to be derived from the host I am using, so that the same code works on localhost and on the deployed hostname.
14. As an Broadcaster, I want each authorization to create a one-use Authorization Attempt that expires after ten minutes, so that a stale or replayed callback is rejected.
15. As an Broadcaster, I want the Authorization Attempt to be bound to my Access identity, so that a callback completed from a different Access identity is rejected.
16. As an Broadcaster, I want the callback to reject a mismatched state, provider, or callback URI, so that a forged callback cannot install tokens.
17. As an Broadcaster, I want the Worker to exchange the authorization code for tokens using the Provider Credentials, so that I never handle tokens myself.
18. As an Broadcaster, I want the Worker to look up the Connected Account after a successful exchange, so that the Broadcaster Page can show who was authorized.
19. As an Broadcaster, I want a successful callback to redirect me to the Broadcaster Page with a success message, so that I get confirmation without seeing any token material.
20. As an Broadcaster, I want a denied consent, a missing code, an expired attempt, an identity mismatch, or a failed exchange to redirect me to the Broadcaster Page with a specific error message, so that I know what went wrong and can try again.
21. As an Broadcaster, I want a failed callback to leave the previous Connection untouched, so that a botched reconnect does not break a working one.
22. As an Broadcaster, I want a successful reconnect to replace the previous Connection entirely, so that the old account's tokens are gone.
23. As an Broadcaster, I want the browser to never receive access or refresh tokens in any response, so that tokens stay inside the deployment.
24. As an Broadcaster, I want the Connection to keep its previous refresh token when a Provider omits one from a token response, so that Spotify's non-rotating refresh tokens keep working.
25. As an Broadcaster, I want the Connection to keep its previous scopes when a Provider omits them from a refresh response, so that the Broadcaster Page stays accurate.
26. As an Broadcaster, I want the Connection to become Reauthorization Required when a token response carries no refresh token and none was stored, so that the problem is visible rather than silent.
27. As an Broadcaster, I want a refresh to be scheduled five minutes before the access token expires, so that tokens are fresh before anything needs them.
28. As an Broadcaster, I want the scheduled refresh to survive Durable Object eviction, so that a cold start does not lose the schedule.
29. As an Broadcaster, I want a refresh that fails with a network error or a rate limit to be retried after one, two, and four minutes, so that a transient outage recovers on its own.
30. As an Broadcaster, I want a refresh that exhausts short retries or fails for an unknown reason to retry again after ten minutes, so that recovery keeps being attempted without hammering the Provider.
31. As an Broadcaster, I want a refresh rejected with a client error other than rate limiting to mark the Connection Reauthorization Required and cancel further refreshes, so that I am prompted to reconnect instead of the deployment retrying forever.
...
34. As a future feature, I want the request to fail with a Not Configured error when no Connection exists, and a Reauthorization Required error when the Connection is in that state, so that I can surface the right message to the Broadcaster.
...
38. As an Broadcaster, I want to run the Worker locally with a fake Access identity, so that I can iterate on the Broadcaster Page without deploying.
39. As an Broadcaster, I want to run a real Spotify and Twitch authorization locally against callback URLs registered on the same Provider applications as production, so that I can test consent without a second set of Credentials.
40. As an Broadcaster, I want local Durable Object storage to be separate from production storage, so that local experiments never overwrite production Connections.
41. As an Broadcaster, I want Provider Credentials supplied through the same configuration mechanism as the existing Access Credentials, so that there is one way to provide secrets.
42. As an Broadcaster, I want the example environment file to list every required Credential, so that setup on a new machine is a matter of filling it in.
43. As an Broadcaster, I want deployment to keep targeting the `production` stage that is already deployed, so that no resources are duplicated.
...
- `AuthorizationAttempt` carries the random state value, Provider, callback URI, the Broadcaster's Access `user_uuid` and email, creation time, expiry time, and a consumed flag.
- `BroadcasterIdentity` is the Access `user_uuid` and email.
...
- The Durable Object hosts the entire layer graph. It exposes RPC methods to start an Authorization Attempt, complete one, get a valid access token, and describe the Connection for the Broadcaster Page. It also owns the alarm handler.
...
- `ConnectionStore`: persistence only, written against Effect's generic `SqlClient`. One row for the Connection as a Schema-validated JSON document, and a table of Authorization Attempts with real columns. Attempt consumption is a single conditional UPDATE on the state value, Provider, callback URI, Broadcaster identity, unconsumed flag, and unexpired time, so it is atomic without an explicit transaction.
...
- `AuthorizationFlow`: starts an attempt for an `BroadcasterIdentity` and returns the Provider redirect URL, and completes an attempt by consuming it, exchanging the code, fetching the Connected Account, and handing the result to `ConnectionLifecycle`.
...
- The Broadcaster Page's update and view are covered by Foldkit's story and scene tests in `apps/web`, with no browser.
...
- Modules under test: the Worker routes, the Broadcaster Page's update and view, `AuthorizationFlow`, `ConnectionLifecycle`, `ConnectionStore`, and both `Provider` implementations. The `SqlClient` adapters themselves are not tested.
