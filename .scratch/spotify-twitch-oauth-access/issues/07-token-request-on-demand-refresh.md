...
Review follow-up: the client-rejection rule moved to `Provider.ts` as `isClientRejection`, next to the failure type it reads; the unreachable "Authorized but no refresh token" branch folded into the one read that yields a refreshable Connection, so such a row is Reauthorization Required at read time without a write; the two read-then-check passes in `requestAccessToken` are one helper applied outside and then inside the lock; the fake Credentials layer is `FakeProviders.layerCredentials` per the layer naming rule; the `Connections` doc says failures arrive as plain tagged objects over the RPC. Added the test both reviewers asked for: a reconnect completing while a refresh is held in flight waits for it, so the Connection ends with the reconnect's tokens and account; removing the lock from `accept` makes it fail. Not changed: `refresh` stays on the service with no caller until ticket 08's alarm, and a rejected refresh leaves `nextRefreshAt` in place for ticket 08 to clear along with the alarm.
