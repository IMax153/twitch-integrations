...
Follow-up, 2026-09-11 (ticket 09): the object now does re-arm the alarm from the stored `nextRefreshAt` when it is constructed, through `ConnectionLifecycle.resumeSchedule`, once the live path had been verified. Arming for a time already armed changes nothing, and a stored time in the past rings at once.
