...
The scope bullet stays open: the reference lists were discarded with the reference document, so nobody has agreed them. The code carries read-only placeholders in `Provider.ts`, Spotify `user-read-currently-playing` and `user-read-playback-state`, Twitch `channel:read:redemptions` and `chat:read`. Settle the lists and tick the bullet before the first real authorization; a widened list only takes effect on the next one.
