...
- [x] The domain package gains `Connection`, `ConnectedAccount`, `AuthorizationAttempt`, and `OperatorIdentity` Schemas plus tagged domain errors, with tokens as Redacted values
...
- [x] Attempt consumption is a single conditional UPDATE matching state, Provider, callback URI, Operator identity, unconsumed, and unexpired, and reports whether a row was consumed
...
- [x] The DO exposes a `describe` RPC returning what the Operator Page needs
...
- [x] The Operator Page shows the status returned by `describe`
