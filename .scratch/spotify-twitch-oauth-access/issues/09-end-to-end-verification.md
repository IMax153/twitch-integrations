...
**What to build:** The Operator can run the real authorization for both Providers on their machine, then deploy to the existing production stage and see both Connections Authorized on the deployed Operator Page. Registering callback URIs and approving consent are human steps; this ticket carries a checklist for them.
...
- [ ] Human steps: run `alchemy dev`, open the Operator Page, complete consent for both Providers, confirm Authorized with the expected Connected Account
