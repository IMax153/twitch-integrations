...
- The Worker has `workersDev: false`.
- A Cloudflare rate-limiting rule guards the route. It is an ordinary stack resource: `Cloudflare.Ruleset.Ruleset` owns the zone's `http_ratelimit` phase and holds one rule that blocks a source IP for ten seconds once it has sent a hundred requests to `/eventsub*` within ten seconds. The Free plan's expression can name only the path, so the rule covers that prefix on every hostname in the zone, where the receiver is the only thing on it. The zone is on the Free plan, which allows one rule, a path-only expression, counting by source IP, and a fixed ten-second period and timeout; Cloudflare keeps the counter per colocation and requires `cf.colo.id` beside the source IP in the rule's characteristics, so a flood spread across data centres is counted at each one separately; a hundred is far above what one channel's Redemptions, retries, and revocations can produce from one address, and a false positive costs one address ten seconds of refusals that Twitch retries. The rule runs at the edge, so a flood from one address costs no Worker requests. It does not stop a flood spread across many addresses; the receiver's own 403s and 404s are the answer to that. The zone enters the stack as an adopted `Cloudflare.Zone.Zone`, retained on destroy by Alchemy's default: the stack never creates or deletes it, and adoption keeps its type and pause settings as they are. Deploying the rule needs the `zone-waf.write` OAuth scope on the Alchemy Cloudflare profile.
